...
noeud* creerNoeudcons(struct list_obj*nodeg,struct list_obj* nodd,int cons){
		noeud* new_ele = (noeud*)malloc(sizeof(struct list_obj*));			
...
noeud* creerNoeudide(struct list_obj*nodeg,struct list_obj* nodd,char* n_var){
	noeud* new_ele = (noeud*)malloc(sizeof(struct list_obj*));			
		...
		new_ele->fils_gauche = nodeg;
		new_ele->fils_droit = nodd;
		if(strcmp(n_var,":=")!=0 || strcmp(n_var,"*")!=0 || strcmp(n_var,"-")!=0 || strcmp(n_var,"+")!=0 || strcmp(n_var,";")!=0 ||strcmp(n_var,"skip")!=0 || strcmp(n_var,"else")!=0 || strcmp(n_var,"while")!=0 || strcmp(n_var,"do")!=0 || strcmp(n_var,"then")!=0 || strcmp(n_var,"else")!=0 )
			{new_ele->type =2;}
		else 
			new_ele->type = 3;			
	return new_ele;
	...
	switch(n->type) {
		...
		if(strcmp(n->data.var,"if")==0){
			if(traitement(e,n->fils_gauche)) {
                        	traitement(e,n->fils_droit->fils_gauche);
                        } 
			else {
	                        traitement(e,n->fils_droit->fils_droit);
                        }
			return 0;
		}
		if(strcmp(n->data.var,":=")==0){	
			initenv(e,n->fils_gauche->data.var);
